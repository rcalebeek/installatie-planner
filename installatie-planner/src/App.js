...
import { Upload, Square, Save, FolderOpen, FileDown, X, Edit2, Trash2, Settings, Cloud, HardDrive } from "lucide-react"
...
export default function App() {
  ...
  const handleCanvasClick = (e) => {
    ...
    if (selectedTool === "delete") {
      const clicked = annotations.find(
        (ann) => Math.sqrt(Math.pow(ann.x - coords.x, 2) + Math.pow(ann.y - coords.y, 2)) < 30,
      )
    ...
    if (selectedTool === "room") {
      if (!isDrawingRoom) {
        ...
        setNewRoomData({
          x: Math.min(roomStart.x, coords.x),
          y: Math.min(roomStart.y, coords.y),
          width: Math.abs(coords.x - roomStart.x),
          height: Math.abs(coords.y - roomStart.y),
        })
      ...
      setAnnotations([
        ...annotations,
        {
          ...
          y: coords.y,
          roomId: room?.id || null,
          roomName: room?.name || "Ongedefinieerd",
        },
      ])
    }
  }
  ...
  const saveProject = async () => {
    ...
    if (useCloud && supabaseClient) {
      ...
      const updated =
        idx >= 0 ? [...projects.slice(0, idx), projectWithId, ...projects.slice(idx + 1)] : [...projects, projectWithId]
  ...
  const deleteRoom = (id) => {
    ...
    setAnnotations(
      annotations.map((ann) => (ann.roomId === id ? { ...ann, roomId: null, roomName: "Ongedefinieerd" } : ann)),
    )
  }

  const confirmRoomName = () => {
    if (roomNameInput.trim() && newRoomData)
      setRooms([...rooms, { id: Date.now(), name: roomNameInput.trim(), ...newRoomData }])
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-lg p-6 max-w-2xl w-full">
            ...
            <div className="space-y-4">
              <div>
                <h3 className="font-bold mb-2">Opslag</h3>
                <div className="flex gap-4">
                  <label className="flex items-center gap-2">
                    <input type="radio" checked={!useCloud} onChange={() => setUseCloud(false)} />
                    <HardDrive size={20} />
                    Lokaal
                  </label>
                  <label className="flex items-center gap-2">
                    <input type="radio" checked={useCloud} onChange={() => setUseCloud(true)} />
                    <Cloud size={20} />
                    Cloud
                  </label>
                </div>
              ...
              {useCloud && (
                <>
                  <div>
                    <label className="block font-semibold mb-2">Supabase URL</label>
                    <input
                      type="text"
                      value={supabaseUrl}
              ...
              <div className="flex gap-2">
                <button onClick={saveSettings} className="flex-1 px-4 py-2 bg-blue-600 text-white rounded-lg">
                  Opslaan
                </button>
                <button onClick={() => setShowSettings(false)} className="px-4 py-2 bg-gray-300 rounded-lg">
                  Annuleren
                </button>
              </div>
            </div>
      ...
      {showProjects && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-lg p-6 max-w-4xl w-full max-h-[80vh] overflow-auto">
            ...
            {isLoading ? (
              <p className="text-center py-8">Laden...</p>
            ) : projects.length === 0 ? (
              <p className="text-center py-8">Geen projecten</p>
            ) : (
                ...
                {projects.map((p) => (
                  <div key={p.id} className="border rounded-lg p-4">
                    ...
                    <p className="text-sm text-gray-600 mb-3">
                      {p.annotations?.length || 0} annotaties, {p.rooms?.length || 0} ruimtes
                    </p>
                    <div className="flex gap-2">
                      <button
                      ...
                      <button onClick={() => deleteProject(p.id)} className="px-3 py-2 bg-red-600 text-white rounded">
                        <Trash2 size={18} />
                      </button>
                    </div>
                  </div>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-lg p-6 max-w-md w-full">
            ...
            <input
              ...
              onKeyPress={(e) => e.key === "Enter" && confirmRoomName()}
              placeholder="bijv. Woonkamer"
              className="w-full px-4 py-2 border rounded-lg mb-4"
              autoFocus
            />
            <div className="flex gap-2">
              <button onClick={confirmRoomName} className="flex-1 px-4 py-2 bg-blue-600 text-white rounded-lg">
                Bevestigen
              </button>
              <button onClick={() => setShowRoomModal(false)} className="px-4 py-2 bg-gray-300 rounded-lg">
                Annuleren
              </button>
            </div>
          </div>
          ...
          <div className="w-80 bg-white shadow-lg p-4 overflow-auto">
            <div className="mb-6">
              <div className="flex justify-between items-center mb-3">
                <h3 className="font-bold text-lg">Annotaties</h3>
                <button onClick={() => setShowLegendEdit(!showLegendEdit)} className="p-1 hover:bg-gray-100 rounded">
                  <Edit2 size={18} />
                </button>
              </div>
              {showLegendEdit && (
                  ...
                  {Object.entries(legend).map(([k, v]) => (
                    <div key={k} className="mb-2">
                      ...
                      <input
                        type="text"
                        value={v}
                        onChange={(e) => setLegend({ ...legend, [k]: e.target.value })}
              ...
              <div className="space-y-2">
                {Object.entries(COLORS).map(([letter, color]) => (
                  <button
                    key={letter}
                    onClick={() => setSelectedTool(selectedTool === letter ? null : letter)}
                    className={`w-full p-3 rounded-lg border-2 ${selectedTool === letter ? "border-blue-600 shadow-lg" : "border-gray-300"}`}
                  ...
                  >
                    <div className="flex items-center justify-between">
                      ...
                      <span className="text-sm">{legend[letter]}</span>
                    </div>
                  </button>
                ))}
              </div>
            ...
            <div className="mb-6">
              <h3 className="font-bold text-lg mb-3">Ruimte Tekenen</h3>
              <button
                onClick={() => {
                ...
                className={`w-full p-3 rounded-lg border-2 mb-2 ${selectedTool === "room" ? "border-blue-600 bg-blue-50" : "border-gray-300"}`}
              >
                <div className="flex items-center gap-2">
                  <Square size={20} />
                  <span>Ruimte definiëren</span>
                </div>
              </button>
              <button
                onClick={() => setSelectedTool(selectedTool === "delete" ? null : "delete")}
                className={`w-full p-3 rounded-lg border-2 ${selectedTool === "delete" ? "border-red-600 bg-red-50" : "border-gray-300"}`}
              >
                <div className="flex items-center gap-2">
                  <Trash2 size={20} />
            ...
            {rooms.length > 0 && (
              <div>
                <h3 className="font-bold text-lg mb-3">Ruimtes</h3>
                <div className="space-y-2">
                  {rooms.map((r) => (
                    <div key={r.id} className="flex items-center justify-between p-2 bg-gray-50 rounded">
                      <span className="font-semibold">{r.name}</span>
                      <button onClick={() => deleteRoom(r.id)} className="p-1 text-red-600 hover:bg-red-50 rounded">
                        <Trash2 size={16} />
                      </button>
                    </div>
                  ))}
        ...
        <div className="flex-1 overflow-auto p-4">
          {uploadedImage ? (
            <canvas
              ...
              className="max-w-full border-2 border-gray-300 rounded-lg shadow-lg cursor-crosshair"
            />
          ) : (
            <div className="h-full flex items-center justify-center text-gray-400">
              <div className="text-center">
                <Upload size={64} className="mx-auto mb-4" />
                <p className="text-xl">Upload een plattegrond om te beginnen</p>
              </div>
            </div>
          )}
        </div>
